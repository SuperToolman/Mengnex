...
const defaults: AgentPolicySettings = { executionMode: "approve_high_risk", allowedCapabilities: ["media.catalog.read", "media.tasks.read"] };
...
export class FileExecutionPolicy extends ExecutionPolicy {
  ...
  constructor(ctx: Context, private readonly filePath = join(process.cwd(), "data", "policy.json")) { super(ctx, "policy"); }
  async load() { try { this.value = normalize(JSON.parse(await readFile(this.filePath, "utf8"))); } catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; } }
