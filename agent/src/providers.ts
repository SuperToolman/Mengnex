...
export class FileProviderRegistry extends ProviderRegistry {
  ...
  constructor(ctx: Context, private readonly filePath = join(process.cwd(), "data", "providers.json"), private readonly legacyPath = join(process.cwd(), "data", "settings.json")) { super(ctx, "providers"); }
  async load() {
    try { this.providers = normalizeProviders(JSON.parse(await readFile(this.filePath, "utf8"))); }
    catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
      this.providers = await this.readLegacy();
      if (this.providers.length) await this.save();
    }
    this.ensureDefault();
  }
  ...
  private async readLegacy() { try { return normalizeProviders(JSON.parse(await readFile(this.legacyPath, "utf8"))); } catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return []; throw error; } }
