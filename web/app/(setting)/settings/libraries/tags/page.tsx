...
const inputClass = "h-10 border border-border bg-white/5 px-3 text-sm text-foreground shadow-none outline-none";
...
export default function TagLibraryPage() {
    ...
    return (
        <SettingsPage
            ...
            actions={<><TextField.Root value={query} onChange={setQuery} className="w-full sm:w-72"><Input aria-label="搜索标签" placeholder="搜索标签" className={inputClass} /></TextField.Root><Button variant="danger" onPress={() => clearModal.open()} isDisabled={busy || tags.length === 0}>清空标签</Button></>}
            contentClassName="space-y-5"
        >
            <Form className="flex flex-col gap-3 sm:flex-row" onSubmit={(event) => { event.preventDefault(); void create(); }}>
                <TextField.Root value={newName} onChange={setNewName} className="min-w-0 flex-1">
                    <Input aria-label="新标签名称" placeholder="新标签名称" maxLength={64} className={inputClass} />
                </TextField.Root>
                <Button type="submit" isDisabled={busy || !newName.trim()} className="gap-2 rounded-lg bg-accent px-4 text-accent-foreground">
                    {busy ? <Spinner size="sm" /> : <Plus className="h-4 w-4" />}添加标签
                </Button>
            </Form>
            ...
            <Drawer.Root state={detailDrawer}>
                <Drawer.Backdrop className="!fixed !inset-0 !z-50">
                    <Drawer.Content placement="right" className="!fixed !inset-y-0 !right-0 !left-auto !m-0 !h-dvh !w-[min(32rem,100vw)] !max-w-none">
            ...
            <Modal state={deleteModal}>
                <Modal.Trigger aria-label="打开删除标签确认对话框" className="sr-only"><button type="button" aria-label="打开删除标签确认对话框" /></Modal.Trigger>
                <Modal.Backdrop isDismissable={!busy} variant="blur" className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <Modal.Container placement="center" className="w-[min(480px,calc(100vw-32px))]">
                        <Modal.Dialog className="outline-none">
                            <Modal.Header className="border-b border-border px-6 py-5"><div><Modal.Heading className="text-lg font-semibold">删除标签</Modal.Heading><p className="mt-1 text-sm text-muted">删除后无法恢复。</p></div><Modal.CloseTrigger aria-label="关闭" className="rounded-md p-2 hover:bg-white/10"><Xmark className="h-5 w-5" /></Modal.CloseTrigger></Modal.Header>
                            ...
                            <Modal.Footer className="flex justify-end gap-3 border-t border-border px-6 py-4"><Button variant="secondary" onPress={() => deleteModal.close()} isDisabled={busy}>取消</Button><Button variant="danger" onPress={() => void remove()} isDisabled={busy || !deleteTarget}>{busy ? <Spinner size="sm" /> : "删除标签"}</Button></Modal.Footer>
                        </Modal.Dialog>
                    </Modal.Container>
                </Modal.Backdrop>
            </Modal>
            <Modal state={clearModal}><Modal.Trigger aria-label="打开清空标签确认对话框" className="sr-only"><span /></Modal.Trigger><Modal.Backdrop isDismissable={!busy} variant="blur" className="fixed inset-0 z-[60] flex items-center justify-center p-4"><Modal.Container placement="center" className="w-[min(480px,calc(100vw-32px))]"><Modal.Dialog><Modal.Header><Modal.Heading>清空标签库</Modal.Heading></Modal.Header><Modal.Body>将删除全部标签及其所有资源关联，此操作无法恢复。</Modal.Body><Modal.Footer className="justify-end gap-3"><Button variant="secondary" onPress={() => clearModal.close()} isDisabled={busy}>取消</Button><Button variant="danger" onPress={() => void clear()} isDisabled={busy}>{busy ? <Spinner size="sm" /> : "确认清空"}</Button></Modal.Footer></Modal.Dialog></Modal.Container></Modal.Backdrop></Modal>
        </SettingsPage>
    );
}
