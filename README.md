# Mengnex

Mengnex 是一个本地优先的媒体管理项目，设计目标参考了 Emby、Steam 以及个人本地媒体库工具。项目希望统一管理多种媒体类型，并逐步覆盖桌面端与 Web 端使用场景，包括照片、游戏、漫画、动漫、电影、剧集、小说和音乐等。

## 当前范围

- 在 Web 设置页管理本地媒体库
- 支持一键启动前后端，并在单终端内查看两个进程日志
- 扫描本地文件系统路径，并将标准化后的媒体记录写入 SQLite
- 提供完整的照片媒体库流程，包括图库浏览、大图预览与原图渐进加载
- 提供照片文件夹浏览视图，支持递归文件夹分组、面包屑导航、拼图封面与目录内搜索
- 为照片生成并管理 `preview` 缓存，加快列表与查看器加载
- 通过统一任务中心跟踪扫描、媒体信息生成等后台任务
- 支持后台任务暂停、继续、取消
- 支持本地目录与 WebDAV 远程数据源
- 支持视频浏览、播放、播放进度、封面生成与视频集合
- 支持音乐媒体库的音频标签解析、专辑/艺人聚合、封面和播放进度
- 支持小说媒体库的 EPUB/TXT 扫描、元数据与章节解析、封面缓存和阅读进度
- 支持账户、角色权限和媒体库级访问隔离

## 项目结构

```text
.
|-- api/                  # Rust Axum Web API
|   |-- data/             # SQLite 数据库与预览图缓存目录
|   `-- src/
|       |-- core/         # 路由、错误处理、健康检查、OpenAPI
|       |-- infra/        # SQLite/SeaORM 连接、实体、基础设施
|       `-- modules/      # auth、libraries、media、photos、videos、music、manga、novels、scanner、tasks 等
|-- web/                  # Next.js Web 客户端
|   |-- app/              # App Router 页面
|   |-- openapi/          # 导出的 OpenAPI 规范
|   `-- src/api/          # 生成后的 API 客户端与手写封装
|-- CHANGELOG.md
`-- .gitignore
```

## 后端

环境要求：

- Rust 1.91+

运行：

```powershell
cd api
cargo run
```

默认地址：

- API: `http://127.0.0.1:3001`
- Swagger UI: `http://127.0.0.1:3001/docs`
- OpenAPI JSON: `http://127.0.0.1:3001/openapi.json`

首次启动不会创建固定默认账户。打开 Web 页面后按初始化界面创建第一个 Owner 账户。通过 HTTPS 或反向代理部署时必须设置 `COOKIE_SECURE=true`；日志级别可通过 `RUST_LOG` 调整。

视频浏览缓存使用 FFmpeg 按视频抽帧，视频技术信息读取使用 FFprobe 获取时长、分辨率和音视频编码。二者需要相应程序位于系统 `PATH`，也可以在扫描设置中配置可执行文件路径；缺失时任务会明确标记为失败并显示原因。

数据目录：

- `api/data/app.db`: SQLite 应用数据库
- `api/data/preview/`: 按媒体库分组的派生缓存目录，包含照片预览图、视频封面、小说封面和音乐专辑封面
- `api/data/avatars/`: 用户、作者与标签头像
- `api/data/webdav-transient/`: 远程媒体处理过程中生成的临时文件

数据库、预览图、头像、WebDAV 临时文件等新增运行数据均已配置 Git 忽略规则。

## 一键启动

在仓库根目录可直接运行：

```powershell
.\start.ps1
```

行为说明：

- 同时启动 `api` 和 `web`
- 两个进程的日志都会显示在当前终端
- 关闭脚本或按 `Ctrl+C` 时，会一并结束前后端进程

## 前端

环境要求：

- Node.js 22.18+
- pnpm

运行：

```powershell
cd web
pnpm install
pnpm dev
```

如果 API 没有运行在 `http://127.0.0.1:3001`，可设置：

```powershell
$env:NEXT_PUBLIC_API_BASE_URL="http://127.0.0.1:3001"
```

## API 访问层

Web 端当前通过 `web/src/api/client.ts` 维护手写的类型和请求封装。业务代码应优先依赖这一层，而不是直接耦合底层生成物或 `fetch` 调用细节。

## 功能概览

### 账户与权限

- 首次打开 Web 页面时创建第一个 Owner，不提供固定默认密码
- 支持 Owner、Admin、Editor、Viewer 四类角色及系统内置权限
- Owner 与 Admin 可访问全部媒体库；Editor 与 Viewer 仅能访问创建账户时授权的媒体库
- 会话 Cookie 使用 `HttpOnly` 与 `SameSite=Lax`，HTTPS 部署时通过 `COOKIE_SECURE=true` 启用 `Secure`

### 媒体库

- 从设置页的 `媒体库 -> 媒体库列表` 创建和管理媒体库
- 修改媒体库名称、根路径、启用状态，以及“扫描后自动补齐缓存”的行为
- 按需重新扫描媒体库
- 在信息弹窗中查看资源数量、缓存占用和生成状态
- 删除媒体库记录、扫描索引和缓存，但不删除原始媒体文件
- 设置页已升级为一级分组加二级菜单结构，当前包含 `媒体库`、`首选项`、`安全`

### 扫描设置

- 在 `媒体库 -> 扫描设置` 中统一配置预览图生成参数
- 支持设置预览图质量和最大分辨率
- 默认最大分辨率为 `640px`、质量为 `45`；项目只生成一份 `preview`，不会额外生成独立缩略图
- 这些参数通过 `/api/preferences` 持久化到数据库
- 后续扫描任务和手动生成缓存任务都会读取这组全局参数

### 照片缓存

- 在 `api/data/preview` 下生成 `preview`
- 在 `photo_assets` 中记录缓存路径、大小与生成时间
- 统一使用 WebP 作为当前主缓存格式
- 列表页使用 `preview` 缓存
- 大图查看时先显示 `preview`，原图加载完成后再渐进替换
- 媒体响应支持 ETag 条件缓存，重复访问未变化资源时不会重复传输内容
- 媒体信息生成与浏览缓存删除可独立于媒体库创建流程单独执行

### 照片浏览

- `/photo` 提供按批次分组的图库视图，共享顶部导航、搜索与内容缩放操作
- `/photo/folder` 提供基于 `source_path` 的递归文件夹视图，支持文件夹与照片混合展示
- 文件夹卡片支持拼图式封面和子项计数，照片与文件夹都可通过头部搜索快速过滤
- 图片查看器提供右上角操作栏，支持信息查看、原图下载、真实删除和自动隐藏交互

### 视频

- `/video` 提供视频目录、搜索、排序、观看状态筛选和分页
- `/video/{id}` 提供视频播放、进度保存、前后视频切换和集合成员切换
- “生成媒体信息”会根据媒体类型选择实现：照片缩放编码为 Preview；视频先通过 FFprobe 读取时长、分辨率和音视频编码，再通过 FFmpeg 为缺少封面的影片抽帧
- 支持按目录规则建立视频集合

### 音乐

- 在设置页创建 `music` 类型媒体库，扫描 MP3、FLAC、M4A、AAC、OGG、Opus 和 WAV
- 扫描先建立音频文件索引；“生成媒体信息”任务再读取标题、艺人、Album Artist、专辑、曲号、年份、流派、作曲/作词/制作人、内嵌封面与音频质量信息
- 支持多艺人关联、独立艺人视图、收藏、最近播放和用户歌单 API
- `/music` 提供首页、专辑、歌曲、艺人、文件夹、收藏、最近播放和歌单视图；歌曲支持标题/艺人/专辑/流派搜索、流派与年份筛选、排序和分页
- 全局播放器跨页面保持播放队列，支持顺序/随机/单曲循环、音量、进度拖动、继续播放和基于内嵌歌词或本地同名 `.lrc` 的滚动歌词提示
- 专辑封面支持 JPEG、PNG、WebP、GIF；歌曲详情展示编码、码率、采样率和位深
- `GET /api/music/tracks/{id}/metadata-candidates` 提供 MusicBrainz 候选匹配，仅返回结果和置信度，不会自动覆盖本地标签；后续可通过统一 Provider 接入授权的中文或私有数据源
- `GET /api/music/tracks/{id}/stream` 可在本地音频上按需转码为 AAC 或 Opus，支持 64-320 kbps；WebDAV 音频转码暂未启用
- 音乐专属表采用启动时破坏性重建策略。每次 API 启动会清空音乐索引、收藏、歌单和播放记录，随后需重新扫描音乐媒体库；原始媒体文件不会受影响

### 小说

- 在设置页创建 `novel` 类型媒体库，当前支持 EPUB 与 TXT 文件。
- 扫描先建立共享媒体索引；“生成媒体信息”任务会解析 EPUB 的标题、作者、语言、简介、封面与章节书脊，TXT 以全文章节形式提供阅读。
- `/novel` 提供支持缩放与搜索的小说书架；`/novel/{id}` 提供详情与章节目录；`/novel/read/{id}` 提供章节阅读、字号调节和自动保存的用户级阅读进度。
- EPUB 内部 HTML 会由后端转换为纯文本章节响应，不直接交给浏览器渲染；当前不支持 PDF、MOBI、AZW、全文搜索、书签或批注。
- 小说支持本地目录与 WebDAV 数据源。远程 EPUB 在解析和阅读时会临时物化到本地，不长期保存原始文件；单个待解析文件上限为 32 MB。

### 远程数据源

- 在 `媒体库 -> 远程数据源` 中配置和验证 WebDAV 连接
- WebDAV 媒体支持远程扫描和按需读取，长期仅保存本地 Preview、视频封面和小说封面等派生资源
- 远程媒体路径会限制在已配置连接根目录内

### 任务中心

- 在统一任务中心中查看媒体库扫描和媒体信息生成任务
- 后端通过 `/api/tasks` 输出统一任务数据
- 扫描只负责建立媒体索引；扫描成功且已启用自动生成时，会继续创建独立的“生成媒体信息”任务
- 媒体信息任务统一使用 `generate_cache` 类型，再根据媒体类型分派照片 Preview、视频技术信息读取与封面生成、音乐标签与专辑封面解析，或小说元数据、章节与封面解析
- 任务会持久化到数据库，失败原因和处理明细可在任务中心查看，应用重启后历史仍可查看
- 支持暂停、继续、取消运行中的后台任务
- 应用重启时会将失去执行器的未完成任务标记为失败，避免任务永久占用媒体库

### 分页与性能

- `/api/photos` 支持基于 `before_id` 的稳定游标分页，同时保留 `limit` / `offset` 兼容参数
- 照片页使用游标分批加载，翻页成本不会随媒体库规模持续增加
- 视频目录的搜索、观看状态、集合代表项、排序、计数和分页均在 SQLite 中完成
- 设置页通过 `/api/libraries/covers` 一次获取各媒体库封面，避免按媒体库产生请求扇出
- WebDAV 派生图源文件分块写入临时文件并在生成后清理；原图按需流式读取，视频 Range 请求保持直连
- SQLite 使用 12 连接上限，并为池内连接统一启用 WAL、NORMAL synchronous、busy timeout 和外键检查

## 质量检查

仓库通过 `.github/workflows/ci.yml` 自动执行以下检查：

- `cargo fmt --check`
- `cargo test --locked`
- `cargo clippy --locked --all-targets --all-features -- -D warnings`
- `pnpm lint`
- `pnpm build`

## 说明

- `media_items` 是跨媒体类型的共享索引，不是每种媒体类型的最终详情表
- 各媒体类型应拥有独立明细表，例如当前的 `photo_assets`，以及未来可能增加的 `game_assets`、`movie_assets` 等
- 当前任务系统已统一为持久化任务模型，扫描和媒体信息生成都通过后台任务执行
- 扫描中的多表写入与删库时的多表删除已纳入事务，以降低半成功状态风险
